...
// --- API Setup ---
const apiKey = "AIzaSyCd6QYhaxSFo8hZx_Me8YLWx0TVkUHp6a4"; // ใส่ Gemini API Key ตรงนี้
  ...
  const renderDashboard = () => (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="bg-white rounded-3xl p-5 shadow-sm border border-gray-100">
        <h3 className="text-sm font-semibold text-gray-800 mb-3 flex items-center gap-2"><Plus size={16} className="text-emerald-500"/> บันทึกรายจ่ายใน Wallet</h3>
        {selectedImage && (
          <div className="relative w-24 h-24 mb-3 rounded-xl overflow-hidden border border-gray-200"><img src={selectedImage} alt="Receipt" className="w-full h-full object-cover" /><button onClick={() => { setSelectedImage(null); if(fileInputRef.current) fileInputRef.current.value = ''; }} className="absolute top-1 right-1 bg-black/50 text-white p-1 rounded-full hover:bg-black/70"><X size={14} /></button></div>
        )}
        <div className="flex items-center gap-2">
          <input type="file" accept="image/*" className="hidden" ref={fileInputRef} onChange={handleFileChange} />
          <button onClick={() => fileInputRef.current?.click()} className="p-3 bg-emerald-50 text-emerald-600 rounded-xl hover:bg-emerald-100 transition-colors"><Camera size={22} /></button>
          <div className="flex-1 bg-gray-50 rounded-xl flex items-center px-4 border border-gray-100 focus-within:border-emerald-300 focus-within:ring-2 focus-within:ring-emerald-100 transition-all"><input type="text" placeholder="เช่น ค่าไฟบ้าน 1200 บาท" className="w-full bg-transparent border-none focus:outline-none py-3 text-sm" value={inputText} onChange={(e) => setInputText(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleAddExpense()} /></div>
          <button onClick={handleAddExpense} disabled={isLoading || (!inputText.trim() && !selectedImage)} className="p-3 bg-emerald-500 text-white rounded-xl hover:bg-emerald-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">{isLoading ? <Loader2 size={22} className="animate-spin" /> : <Send size={22} />}</button>
        </div>
      </div>
                  ...
                  return (
                    <div key={group.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 hover:border-emerald-100 transition-colors overflow-hidden">
                      <div className="flex justify-between items-center p-4 cursor-pointer hover:bg-gray-50 transition-colors" onClick={() => toggleGroup(group.id)}>
                        ...
                        <div className="flex items-center gap-3 shrink-0"><p className="font-bold text-gray-800">-฿{groupTotal.toLocaleString()}</p><div className="text-gray-400 bg-gray-50 p-1 rounded-full">{isExpanded ? <ChevronUp size={16} /> : <ChevronDown size={16} />}</div></div>
                      </div>
                      {isExpanded && (
      ...
      <div className="w-full max-w-md bg-[#f8fafc] p-4 flex flex-col relative">
        ...
        <main className="flex-1">{activeTab === 'home' ? renderDashboard() : (
          <div className="flex flex-col h-[calc(100vh-140px)] animate-in fade-in duration-300">
            ...
            <div className="bg-white rounded-2xl p-2 border border-gray-200 shadow-sm flex items-center mt-auto"><input type="text" placeholder="พิมพ์คำถามที่นี่..." className="flex-1 bg-transparent border-none focus:outline-none px-4 py-2 text-sm" value={chatInput} onChange={(e) => setChatInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSendMessage()} /><button onClick={handleSendMessage} disabled={isLoading || !chatInput.trim()} className="p-3 bg-emerald-500 text-white rounded-xl hover:bg-emerald-600 disabled:opacity-50 transition-colors"><Send size={18} /></button></div>
          </div>
        )}</main>
